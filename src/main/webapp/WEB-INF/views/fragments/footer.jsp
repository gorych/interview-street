<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<footer class="page-footer teal">
    <div class="container">
        <div class="row">
            <div class="col l9 s12">
                <h5 class="white-text">Interview Street</h5>
                <p class="grey-text text-lighten-4">Это бесплатный онлайн-опросник, который позволяет создавать, редактировать и проходить анкеты с профессиональным оформлением и
                    содержанием.</p>
            </div>
            <div class="col l3 s12 white-text">
                <h5>Ссылки</h5>
                <ul>
                    <li><a class="white-text" href="https://www.gstu.by">Сайт университета</a></li>
                    <li><a class="white-text" href="http://www.edu.gstu.by/">Учебный портал</a></li>
                    <li><a class="white-text" href="#!">GitHub</a></li>
                </ul>
            </div>
        </div>
    </div>
    <div class="footer-copyright">
        <div class="container">
            &copy;Егор Семенченя, ГГТУ 2016<a class="brown-text text-lighten-3" href="#"></a>
        </div>
    </div>
</footer>